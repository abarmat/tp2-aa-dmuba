...
probs <- matrix(results, nrow=dim(df_all)[1], ncol=5, byrow=TRUE)
preds <- apply(as.matrix(probs), 1, which.max)
all <- cbind(preds, probs)

# Accuracy
sum(as.integer(preds == y_test$Clase+1))/length(preds)

# Out
write.table(all, file="xgboost-pred-test.csv", 
